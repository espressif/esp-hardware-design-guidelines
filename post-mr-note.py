...
def getArgs():
    ...
    Sample usage: python post-mr-note.py [PROJECT-TOKEN] espressif/esp-hardware-design-guidelines 7 https://gitlab.example.cn
    """
    ...
    def prepare_note(self):
        ...
        for chip_series, language_links in self.series_links.items():
            ...
            if 'en' in language_links:
                note += f"[Hardware Design Guidelines]({language_links['en']})"
            else:
                note += "Hardware Design Guidelines"
            note += "\n"
        self.note = note
