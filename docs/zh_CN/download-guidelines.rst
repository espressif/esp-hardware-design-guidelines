...
.. list::
    ...
    3. 使用 :download:`Flash 下载工具 <https://dl.espressif.com/public/flash_download_tool.zip>`，选择 UART 方式将程序固件烧录进 flash 中。
    :not esp32s31: 4. 烧录结束后， {IDF_TARGET_SPI_BOOT_PIN} 可以悬空或者上拉切换至高电平，进入 SPI Boot 启动模式下工作。
    :esp32s31: 4. 烧录结束后，根据表 :ref:`tab-chip-boot-mode-control` 设置芯片在 SPI Boot 模式下工作。
...
.. only:: not esp32 and not esp32c2 and not esp32s2
    ...
    .. list::
        ...
        3. 使用 :download:`Flash 下载工具 <https://dl.espressif.com/public/flash_download_tool.zip>`，选择 USB 方式将程序固件烧录进 flash 中。
        :not esp32s31: 4. 烧录结束后， {IDF_TARGET_SPI_BOOT_PIN} 可以悬空或者上拉切换至高电平，进入 SPI Boot 启动模式下工作。
        :esp32s31: 4. 烧录结束后，根据表 :ref:`tab-chip-boot-mode-control` 设置芯片在 SPI Boot 模式下工作。
        5. 重新上电，芯片/模组初始化时会从 flash 中读取程序运行。
...
.. note::
    ...
    .. list::
        ...
        .. only:: not esp32 and not esp32c2
            ...
            - 应用程序中如果出现以下情况，USB 自动下载功能将被禁用，必须通过配置 strapping 管脚进入 Joint Download Boot 模式，才能使用 USB 下载功能。
            ...
            - 建议用户保留 UART 的下载接口，当前射频测试固件仅支持 UART 接口。
